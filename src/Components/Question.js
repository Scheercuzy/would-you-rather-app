...
class Question extends Component {
    ...
    componentWillMount () {
        ...
        if (questionInfo) {
            const showAnswerInfo = questionInfo.optionOne.votes.includes(authUser) || questionInfo.optionTwo.votes.includes(authUser)
            const currentUserChoice = (!showAnswerInfo) ? null : (questionInfo.optionOne.votes.includes(authUser)) ? 'optionOne' : 'optionTwo'
    ...
    percentagePicked () {
        ...
        const totalVotes = questionInfo.optionOne.votes.length + questionInfo.optionTwo.votes.length
        const percentage = (questionInfo[currentUserChoice].votes.length / totalVotes) * 100
            ...
            <Fragment>
                <Paper>
                    <ListItem key={questionInfo.author} className={classes.author}>
                        <ListItemAvatar>
                            <Avatar alt={questionInfo.author} src={this.handleUserAvatar(questionInfo.author)}/>
                        </ListItemAvatar>
                    <ListItemText primary={questionInfo.author} secondary="Question author"/>
                    </ListItem>
                </Paper>
                ...
                {showAnswerInfo &&
                    <Paper className={classes.infoPaper}>
                        ...
                        <br/>
                        <Typography>You Picked <strong>{questionInfo[currentUserChoice].text}</strong></Typography>
                        <Typography><strong>{this.percentagePicked()}%</strong> of the users picked the same answer</Typography>
                        <Typography><strong>{questionInfo[currentUserChoice].votes.length}</strong> Users picked the same</Typography>
                    </Paper>
                }
            </Fragment>   
        )
